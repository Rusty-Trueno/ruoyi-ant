 ...
 * @returns {Promise}
 */
export const getRouterByUser = (loginInfo) => {
  return axios({
 ...
 * @returns {Promise<any>}
 */
export const generatorDynamicRouter = (loginInfo) => {
  return new Promise((resolve, reject) => {
    console.log(loginInfo)
    // ajax
    getRouterByUser(loginInfo).then(res => {
      const result = res.result
      console.log(result)
      const rootMenu = getRootMenu(result)
...
export function getRootMenu (rows) {
  ...
  buildtree(rows[0].subMenu, arr, 1)
  arr.forEach(row => {
    menus.push(row)
  })
  console.log(menus)
...
export function buildtree (list, arr, parentId) {
  if (list) {
    list.forEach(item => {
      if (parseInt(item.pid) === parentId) {
        var child = {
          title: item.menuName,
          key: item.url ? item.url : '/root',
          icon: 'interaction',
        ...
        buildtree(item.subMenu, child.children, parseInt(item.id))
