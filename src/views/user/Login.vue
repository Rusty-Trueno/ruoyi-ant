...
export default {
  ...
  methods: {
    ...
    handleSubmit (e) {
      ...
      validateFields(validateFieldsKey, { force: true }, (err, values) => {
        if (!err) {
          ...
          loginParams.imageCode = values.captcha
    ...
    async getImgCode (deviceId) {
      await imgcode(deviceId).then(res => {
        const image = 'data:image/jpg;base64,' + res.data.result
        this.codesrc = image
      })
    },
    loginSuccess (res) {
